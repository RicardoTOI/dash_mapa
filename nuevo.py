...
conexion = mysql.connector.connect(
    ...
    database='toicommx_inmobiliaria', port='3306')
...
def mapa(est_selec, mun_selec, zona_selec,m_totales, m_construidos, precio_, tipo_baño_, habitacion, antiguedad, estacionamientos):
    ...
    if zona_selec:
        ...
        for zona in zona_selec:
            ...
            #Marcamos en el mapa
            tipo = df13['Tipo'].tolist()    
            precio = df13['Precios'].tolist() 
            moneda = df13['Moneda'].tolist() 
            habitaciones = df13['Recamaras'].tolist()
            baños = df13['Baños'].tolist()
            totm2=df13['Metros_totales'].tolist()
            constm2=df13['Metros_construidos'].tolist()
            direccion = df13['Direcciones'].tolist()
            antiguedad=df11['Antiguedad'].tolist()
            estacionamientos = df11['Estacionamientos'].tolist()

            for i in range(len(list_lat)):

                if habitaciones[i] == 0.1:
